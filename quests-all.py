...
from circular import rotations
...
init_lines = list(set(lines))
all_paths = dict()

for count, lines in tqdm(enumerate(rotations(init_lines))):
    coords = []
    ...
    if count == 0:
        print(f'{t*len(init_lines)} iterations')
        print(time.strftime('%H:%M:%S', time.gmtime(int((t*len(init_lines))/450000))), end='')
        print()
    random = False
    if seconds_int > 180:
    ...
    all_paths[shortest_size] = points
# pprint(all_paths)
points = all_paths[min(all_paths.keys())]
# print(points)
for x in points:
